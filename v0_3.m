...
% accomplished:
    % gammas
        % multiple interaction cross-sections
        % secondary particles
    % physically useful graphs
        % energy deposition scaling
        % dose
...
imgRaw = imread(fullfile(path, file));
    ...
    if size(imgRaw, 3) == 3
        ...
        % reject images without clear regional boundaries
        % contrast threshold
        %edgeCheck = edge(imgGray, 'Canny');
        %edgeDensity = sum(edgeCheck(:)) / numel(imgGray);
        %if edgeDensity < 0.005
        %    error(' ');
        %end    
            % immediate problem: what if edges do not enclose shapes?
                % edge detection may not be necessary at all actually
    % resize to 1000x1000
...
for r = 1:length(numRegions)
    ...
    promptMat = {sprintf('Enter density for Region %d [g/cm^3]:', regionID), sprintf('Enter effective Z (Atomic Number) for Region %d:', regionID)};
    ...
    materialProps(regionID).density = str2double(answerMat{1});
    materialProps(regionID).Z = str2double(answerMat{2});
end
close(gcf);
...
sampleCountText = "Sample count linear density [n/px]" + newline ...
    ...
    + newline + "Recommended: 10 for alphas, 100 for betas, 1000 for gammas";
promptParams = {particleTypeText, 'Particle energy [MeV]:', sampleCountText};
dlgTitleParams = 'Particle Configuration';
defaultParams = {'1', '6.0', '10'}; % 6 MeV alphas, 10000 samples
...
numSamples = samplesPerPixel * gridSize;

% set source parameters (v2)
...
if particleType == 1
    ...
    for i = 1:numSamples
        waitbar(i/numSamples, progressBar, sprintf('Sample %d out of %d', i, numSamples));

        posX = 1;
        posY = randi(1000); % random spread on left end of image
...
elseif particleType == 3
    ...
    % gammas do not lose energy continuously like charged particles
    % calculate free path length using interaction cross-sections
    % the LUT should be free path then instead of stopping power

    E_space = linspace(0.01, initialEnergy, 10000); 
    epsilon = E_space ./ m_e;

    % import everything from cross-section function
    [LUT_GammaPhoto1, ...
        LUT_GammaCompton1, ...
        LUT_GammaPairpro1, ...
        LUT_GammaSigma1, ...
    ...
    % well that's a lot more than free path
    ...
    for i = 1:numSamples
        ...
        while energy > 0
            % what if it crosses multiple materials in one free path?
            % check starting position material 
            % advance by free path of that material
            % check if within simulation boundaries
            % locate boundary/boundaries
                % advance by steps along angle theta and check material
                % for what? not sure how interactions are changed by this
                % other than that a boundary would interrupt the free path
            ...
            % endingMaterial = imgFinal(round(nextY), round(nextX));
                % does not seem to be useful yet

            % advance by free path length
            freePathLength = -log(1 - rand()) / linearAttCoeff;
            freePathPixels = freePathLength / cmPerPixel;
            nextX = posX + freePathPixels * cos(theta);
            nextY = posY + freePathPixels * sin(theta);                
            if nextX < 1 || nextX > gridSize || nextY < 1 || nextY > gridSize
                break;
            end            
            ...
            elseif gammaInteraction > threshCompton
                ...
                % could do secondary Bremsstrahlung/positron annihilation 
                % gammas and tertiary betas and tertiary gammas, but 
                % save that for v2 since that needs better structure
...
figure(1);
%scaleFactor = 255 / max(max(doseMap));
...
if particleType == 3
    ...
    %scaleFactorPE = 255 / max(max(doseMapPhotoelectric));
    ...
    %scaleFactorCompton = 255 / max(max(doseMapCompton));
    ...
    colorbar;
    grid on;
    title('Deposition per Particle from Compton Scattering [MeV]');
    xlabel('cm');
    ylabel('cm');

    %scaleFactorPairpro = 255 / max(max(doseMapPairpro));
    ...
    colorbar;
    grid on;
    title('Deposition per Particle from Pair Production Events [MeV]');
    xlabel('cm');
    ylabel('cm');
end
...
% results need to be multiplied by activity [Bq] [n/s] and time [s] to
% obtain real dose [Gy]
...
function dEdx = calculateAlpha(E_space, materialStruct)
    ...
    % loop method (original)
    % dEdx = zeros(size(E_space));
    %
    % for k = 1:length(E_space)
    %     E = E_space(k);
    %    
    %     v_alpha = sqrt((2 * E) / m_alpha) * c; % [sqrt(MeV / (MeV / c^2)) * (cm/s)/c] = [cm/s], this confused me too much
    %     beta = v_alpha / c; % redundand I guess but maybe I need v_alpha later
    %     betaSq = beta^2;
    ...
    %         (log((2 * m_e * betaSq) / (bethe_I * (1 - betaSq))) - betaSq);
    % 
    %     if stoppingPower > 10000 || stoppingPower < 0
    %         stoppingPower = 10000;
    %     end
    % 
    %     dEdx(k) = stoppingPower; % [MeV / cm] * realLength [cm] = [MeV]
    % end
end
...
% first draft (not functional)
    % % component functions and constants
    % m_e = 0.511; % do i need to say it
    % r_e = 2.82e-13;
    % epsilonZero = m_e / photonEnergy;
    %     % epsilonPairpro >= epsilonZero && epsilonPairpro <= 1 - epsilonZero
    % delta = @(epsilon) 136/(Z^(1/3)) * epsilonZero./(epsilon .* (1 - epsilon));      
    % finestructure = 1/137.036;
    % a = (finestructure * Z)^2;
    % coulombCorrection = a * (1/(1 + a) ...
    %     + 0.20206 ...
    ...
    % if photonEnergy < 50
    %     coulombFunc = 8/3 * log(Z);
    %     phi_1 = @(epsilon) 20.867 - 3.242 * delta + 0.625 * delta.^2;
    %     phi_2 = @(epsilon) 20.209 - 1.930 * delta - 0.086 * delta.^2;
    % elseif photonEnergy >= 50
    %     coulombFunc = 8/3 * log(Z) + 8 * coulombCorrection;
    %     phi_1 = @(epsilon) 21.12 - 4.184 * log(delta + 0.952);
    %     phi_2 = phi_1;
    % end
    % xi = (log(1440/Z^(2/3)) / log(183/Z^(1/3)) - coulombCorrection);
    % 
    ...
    % C = finestructure * r_e^2 * Z * (Z + xi);
    % dSigmadEpsilon = @(epsilon) C * ( ...
    %         (epsilon.^2 + (1 - epsilon).^2) ...
    %         .* (phi_1(delta) - coulombFunc / 2) ...
    %         + 2/3 * epsilon .* (1 - epsilon) ...
    %         .* (phi_2(delta) - coulombFunc / 2)...
    %     );
    % 
    % % trapezoidal rule integration
    % epsilon_space = linspace(epsilonZero, 1 - epsilonZero, 10000);
    % dSigmadEpsilon_space = dSigmadEpsilon(epsilon_space);
    % I = cumtrapz(epsilon_space, dSigmadEpsilon_space);
    % 
    % % normalize
    % CDF = I ./ I(end);
    ...
    % 
    % % find epsilon
    % epsilonPairpro = interp1(CDF, epsilon_space, U);